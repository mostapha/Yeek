...
const computeWindow = document.querySelector("#compute-wrapper")
	? window
	: document.querySelector("#sandbox-maui-preact-container")?.contentWindow;
if (!computeWindow)
	throw new Error("Failed to find iframe window");

const createBtn = computeWindow.document.querySelector('button[aria-label="Create"]');
if (!createBtn || createBtn.textContent !== "Create")
	throw new Error("Failed to find 'Create' button");

const contentsElmt = computeWindow.document.querySelector(".jet-form-layout-content-body");
if (!contentsElmt)
	throw new Error("Failed to find contents element");

const headerElmt = computeWindow.document.querySelector('[data-test-id="page_header"]');
if (!headerElmt)
